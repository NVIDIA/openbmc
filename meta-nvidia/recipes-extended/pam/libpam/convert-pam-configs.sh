...
then
    ...
    sed -i -e "s/^minlen=.*/minlen=$minlen/" ${SECURITY_CONF_DIR}/pwquality.conf
    pwquality='password        [success=ok default=die]        pam_pwquality.so debug'
    sed -i -e "s/^password.*pam_cracklib.so.*/$pwquality/" ${PAM_CONF_DIR}/common-password
...
then
    ...
    sed -i -e "s/^remember=.*/remember=$remember/" ${SECURITY_CONF_DIR}/pwhistory.conf
    pwhistory='password        [success=ok ignore=ignore default=die]  pam_pwhistory.so debug use_authtok'
    sed -i -e "s/^password.*pam_pwhistory.so.*/$pwhistory/" ${PAM_CONF_DIR}/common-password
    echo "# This file was converted by $0" >>${PAM_CONF_DIR}/common-password
fi
