...
DUMP_CFG_INPUT_FILE="/usr/share/device_mcu_eid.csv"
MCU_LOG_FILE="/var/mctp-vdm-output.bin"

function help()
{
    echo "Usage: mcu_dump [-h] -p <file_path> -i <dump_id>"
    echo ""
    echo "Options:"
...
function main()
{
    ...
    do
        ...
        tmpFileName="${name}_mcu_dump.bin"
        mv -f $MCU_LOG_FILE "$TMP_DIR_PATH/$tmpFileName"
    done
