...
cp -rp init shutdown update whitelist reserved-list bin sbin usr lib etc var run/initramfs
...
echo "1e631000.spi" > /sys/bus/platform/drivers/spi-aspeed-smc/unbind
echo "1e630000.spi" > /sys/bus/platform/drivers/spi-aspeed-smc/unbind
echo "1e620000.spi" > /sys/bus/platform/drivers/spi-aspeed-smc/unbind
echo "1e620000.spi" > /sys/bus/platform/drivers/spi-aspeed-smc/bind
echo "1e631000.spi" > /sys/bus/platform/drivers/spi-aspeed-smc/bind
...
elif ! timeout 60 mount $rwdev $rwdir -t $rwfst -o $rwopts
then
	msg="$(cat)" << HERE
...
then
    cd $work
	if ls -a | grep -v -E '^\.$|^\.\.$' | timeout 30 xargs rm -rf
	then
		cd -
	else
		cd -
		echo "Issues detected with RWFS, cant remove $work"
		if mountpoint $rwdir
		then
			timeout 30 umount $rwdev
		fi
	fi
fi
...
HERE
	echo "$msg"
	echo "Failures detected with overlay, booting with ROFS"
	exec chroot $rodir $init
done
logging_dir=/var/lib/logging
mkdir -p $logging_dir
if ! timeout 60 mount -t jffs2 -o defaults,sync  $logdev $logging_dir
then
    echo "FAILED TO MOUNT LOGGING PARTITION!!!!"
else
    echo "Mounting of logging partition completed"
fi
