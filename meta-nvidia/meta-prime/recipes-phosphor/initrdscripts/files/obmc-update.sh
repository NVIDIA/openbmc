...
save_files() {
	...
	do
		...
		if test "/${f#/}" != "$f" -o "${f%/..}" != "${f#*/../}"
		then
			echo 1>&2 "WARNING: Skipping bad $file entry $f."
			continue
		fi
		if ! test -e "$upper/$f"
		then
...
reserved_list=/run/initramfs/reserved-list
...
if test "x$dosave" = xy
then
	save_files "$whitelist"

else
	# Always reserved files based on the reserved-list
	save_files "$reserved_list"
fi
