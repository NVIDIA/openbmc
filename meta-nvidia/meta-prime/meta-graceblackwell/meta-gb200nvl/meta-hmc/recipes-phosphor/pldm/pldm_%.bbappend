...
EXTRA_OEMESON:append  = " -Dlibpldmresponder=enabled "
...
do_install:append() {
    ...
    install -m 0644 ${UNPACKDIR}/fw_update_config_c1g2.json ${D}/etc/pldm/fw_update_config.json
    ln -sf /etc/pldm/fw_update_config.json ${D}${datadir}/pldm/fw_update_config.json
    install -m 0644 ${UNPACKDIR}/fw_update_config_c1g2.json ${D}${datadir}/pldm/platform-config-files
    install -m 0644 ${UNPACKDIR}/fw_update_config_c1g1.json ${D}${datadir}/pldm/platform-config-files
    install -m 0755 ${UNPACKDIR}/pldm_cfg_setup.sh ${D}/${bindir}/pldm_cfg_setup.sh
    install -m 0644 ${UNPACKDIR}/pldm_cfg_setup.service ${D}${base_libdir}/systemd/system
