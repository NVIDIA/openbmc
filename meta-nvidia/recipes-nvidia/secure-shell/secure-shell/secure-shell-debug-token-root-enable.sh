...
get_erot_eid() {
    local MCTP_EPS=$(busctl call xyz.openbmc_project.ObjectMapper /xyz/openbmc_project/object_mapper xyz.openbmc_project.ObjectMapper GetSubTree sias "/" 0 1 xyz.openbmc_project.MCTP.Endpoint | cut -d" " -f3- | tr -d '"')
...
disallow_root_login() {
    echo "root login not permitted" >&2
    exit 1
}

allow_root_login() {
    echo "root login permitted" >&2
    exit 0
}
